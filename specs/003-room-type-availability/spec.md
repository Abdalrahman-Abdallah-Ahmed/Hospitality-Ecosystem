...
- **FR-009**: Any change that adds booked lines to a room type on a night MUST be rejected
  ...
  moving or extending a reservation's dates, and bringing a cancelled reservation back
  (its lines are reinstated, per the SPEC-010 review fix). A line's room type cannot be
  changed (SPEC-010); if a later spec allows it, the same rule applies without changes. The reservation's own
