...
- **Rationale**: The reservation's own current lines never count against it. Legacy nights
  ...
  Reinstating a cancelled reservation through `update()` leaves its lines cancelled today,
  so its footprint stays empty, and `diff()` rejects room-type changes. Neither is a
  trigger in this spec (FR-009). If SPEC-012 revives lines, the same guard applies without
  changes. Re-creating a deleted reservation is a trigger: `create()` treats its old lines
